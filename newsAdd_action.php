...
//this block is called when button Submit is clicked
	...
	//Check if there is an image to be uploaded
		...
		if(isset($_FILES["fileToUpload"]) && $_FILES["fileToUpload"]["name"] == ""){
            $sql = "INSERT INTO news (title, content, newsType, image, date_published) 
            VALUES('" . $title . "', '" . $content . "', " . $type . ", '" . $uploadfileName . "', NOW())";
		...
		//IF there is image
			...
			if($uploadOk){
				$sql = "INSERT INTO news (userID, title, content, newsType, image, date_published) 
                VALUES(" . $_SESSION["UID"] . ", '" . $title . "', '" . $content . "', '" . $type . "', '" . $uploadfileName . "', NOW())";
				$status = insertTo_DBTable($conn, $sql);
					if ($status) {
